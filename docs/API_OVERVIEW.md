...
High-level map of every HTTP endpoint exposed by the backend, the full order flow, the auth model, and the status enums the frontend needs. For **exact request/response schemas**, use the live Swagger UI at **http://localhost:8000/docs** — it is generated from the Pydantic models and is the authoritative source of truth. This document is a narrative companion for human readers.

Related references:
- [ADMIN_API.md](ADMIN_API.md) — admin-only endpoints (separate auth model)
- [SJP_FRONTEND_GUIDE.md](SJP_FRONTEND_GUIDE.md) — SJP (Safe Job Procedure) async generation flow
...
The unauthenticated parts of the order-creation flow (plans, order create, company details, preview, legal, checkout) are public by design — this matches the product flow where a user fills out an order before ever creating an account. Accounts and the dashboard come into play after payment.
